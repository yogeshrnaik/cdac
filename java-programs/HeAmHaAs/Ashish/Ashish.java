...
{  int ino;
  ...
  int time;
  int posi,posj;
  Ashish(SimpleInput sin) throws IOException
  {
    ino = sin.readInt();
    for (int i=0;i<ino;i++)
    {
     ...
     for(int j=0;j<ino;j++)
          {
            if(a[j].type == 'D')
                                  
                        {
                               ...
                               if((a[j].ii>=0)&&(a[j].ii<=9)&&(a[j].ij>=0)&&(a[j].ij<=9))
                        ...
                        {      
                             if(a[j].ii>=0 && a[j].ii<=9 && a[j].ij>=0 &&a[j].ij<=9 && grid[a[j].ii][a[j].ij]>0)
                              { grid[a[j].ii][a[j].ij] -= 1;
